...
export function getInserterItems( state, editorAllowedBlockTypes, parentUID = null ) {
	...
	const parentBlock = getBlock( state, parentUID );
	...
	const calculateUtility = ( category, isContextual, count ) => {
		if ( isContextual ) {
			return 3;
		} else if ( count > 0 ) {
			return 2;
	...
	const canInsertBlockType = ( blockType ) => {
		const isAllowedInEditor = checkAllowList( editorAllowedBlockTypes, blockType.name );
		if ( ! isAllowedInEditor ) {
			return false;
		}

		const isAllowedInPostType = checkAllowList( blockType.allowedPostTypes, postType );
		if ( ! isAllowedInPostType ) {
			return false;
		}

		if ( parentBlock ) {
			const isChildAllowedByParent = checkAllowList( parentAllowedBlockTypes, blockType.name );
			const isParentAllowedByChild = checkAllowList( blockType.allowedParents, parentBlock.name );
			if ( ! isChildAllowedByParent && ! isParentAllowedByChild ) {
				return false;
			}
		}
	...
	const buildBlockTypeInserterItem = ( blockType ) => {
		...
		const isContextual = Array.isArray( blockType.parentAllowedBlockTypes );
		const { time, count } = getInsertUsage( state, id );
		const utility = calculateUtility( blockType.category, isContextual, count );
	...
	const canInsertSharedBlock = ( sharedBlock ) => {
		const blockType = getBlockType( 'core/block' );
		if ( ! canInsertBlockType( blockType ) ) {
			return false;
		}
	...
	const buildSharedBlockInserterItem = ( sharedBlock ) => {
		...
		const { time, count } = getInsertUsage( state, id );
		const utility = calculateUtility( 'shared', false, count );
		const frecency = calculateFrecency( time, count );
